...
described in the specification using OpenMP.
...
this program) compile with:
        ...
        gcc -std=c99 -Wall -Wpedantic -DMOVIE -fopenmp ompcircles.c -o
ompcircles -lm
...
int compute_forces(void) {
  ...
  // start parallelize the loops, add reduction to avoid race condition on
  // n_intersections and the dynamic scheduling to give all threads the same
  // amount of work
#pragma omp parallel for default(none) shared(ncircles, EPSILON, circles, K)   \
    reduction(+ : n_intersections) schedule(dynamic)
  for (int i = 0; i < ncircles; i++) {
    for (int j = i + 1; j < ncircles; j++) {
      ...
      if (dist < Rsum - EPSILON) {
        ...
        // the following instructions have to be atomic to avoid race conditions
#pragma omp atomic
